...
`,mt=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];function gt(t){if(!t||"string"!=typeof t)return!1;const e=new Date(t);return!isNaN(e.getTime())}function bt(t){if(!gt(t))return"Invalid date";const e=new Date(t),r=new Date,s=new Date(e.getFullYear(),e.getMonth(),e.getDate()),i=new Date(r.getFullYear(),r.getMonth(),r.getDate()).getTime()-s.getTime(),o=Math.round(i/864e5);return 0===o?"Today":1===o?"Yesterday":`${e.getDate()} ${mt[e.getMonth()]}`}function vt(t){return"number"==typeof t&&!isNaN(t)&&t>=0}function yt(t){if("string"==typeof t){const e=t.toLowerCase();if("success"===e||"ok"===e||"completed"===e)return"success";if("partial"===e||"warning"===e||"warn"===e)return"partial";if("failed"===e||"fail"===e||"error"===e)return"failed";if("running"===e||"in_progress"===e||"in progress"===e||"ongoing"===e)return"running"}return console.warn(`Invalid outcome value: ${t}, defaulting to 'failed'`),"failed"}const $t={show_header:!0,title:"Backup Status",show_errors:!1};function _t(t,e){const r=[];t.name&&"string"==typeof t.name||r.push(`Entry ${e}: Missing or invalid 'name'`),t.start&&"string"==typeof t.start?gt(t.start)||r.push(`Entry ${e}: Invalid date format for 'start'`):r.push(`Entry ${e}: Missing or invalid 'start'`);const s="number"==typeof t.duration?t.duration:0;if(vt(s)||r.push(`Entry ${e}: Invalid duration`),r.length>0)return{entry:null,error:r.join("; ")};try{return{entry:{name:t.name.trim(),start:t.start,duration:Math.max(0,s),outcome:yt(t.outcome),notes:"string"==typeof t.notes?t.notes.trim():"",size:"string"==typeof t.size?t.size.trim():void 0},error:null}}catch(t){return{entry:null,error:`Entry ${e}: ${t}`}}}function xt(t){const e={entries:[],errors:[]};if(!t)return e.errors.push("Entity state is undefined"),e;if(!t.attributes)return e.errors.push("Entity has no attributes"),e;if(t.attributes.backups&&Array.isArray(t.attributes.backups))return t.attributes.backups.forEach((t,r)=>{const{entry:s,error:i}=_t(t,r);s?e.entries.push(s):i&&e.errors.push(i)}),e;if(t.attributes.name||t.attributes.start){const{entry:r,error:s}=_t(t.attributes,0);return r?e.entries.push(r):s&&e.errors.push(s),e}return e.errors.push("No backup data found in entity attributes"),e}let wt=class extends nt{constructor(){super(...arguments),this.outcome="success"}render(){return L`
      <div class="badge badge-${this.outcome}">
        <ha-icon icon="${function(t){switch(t){case"success":return"mdi:check-circle";case"partial":return"mdi:alert";case"failed":return"mdi:close-circle";case"running":return"mdi:progress-clock";default:return"mdi:help-circle"}}(this.outcome)}"></ha-icon>
            ...
            ${this.backups.map(t=>L`
              <div class="backup-row">
                <div class="table-row">
                  ...
                  <div class="cell-duration">${function(t){if(!vt(t))return"Invalid duration";if(0===t)return"0s";const e=Math.floor(t/3600),r=Math.floor(t%3600/60),s=Math.floor(t%60),i=[];return e>0&&i.push(`${e}h`),r>0&&i.push(`${r}m`),(s>0||0===i.length)&&i.push(`${s}s`),i.join(" ")}(t.duration)}</div>
                  <div class="cell-outcome">
                    <outcome-badge .outcome=${t.outcome}></outcome-badge>
                  </div>
                </div>
